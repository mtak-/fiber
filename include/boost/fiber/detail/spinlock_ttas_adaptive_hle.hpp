...
#ifndef BOOST_FIBERS_SPINLOCK_TTAS_ADAPTIVE_HLE_HLE_H
#define BOOST_FIBERS_SPINLOCK_TTAS_ADAPTIVE_HLE_HLE_H
...
class spinlock_ttas_adaptive_hle {
private:
...
public:
    spinlock_ttas_adaptive_hle() noexcept = default;

    spinlock_ttas_adaptive_hle( spinlock_ttas_adaptive_hle const&) = delete;
    spinlock_ttas_adaptive_hle & operator=( spinlock_ttas_adaptive_hle const&) = delete;
    ...
    void lock() noexcept {
        ...
        for (;;) {
            std::size_t count = 0;
            const std::size_t prev_count = count_.load( std::memory_order_relaxed);
            const std::size_t max_relax_count = (std::min)(
                    static_cast< std::size_t >( BOOST_FIBERS_SPIN_BEFORE_SLEEP0), 2 * prev_count + 10);
            const std::size_t max_sleep_count = (std::min)(
                    static_cast< std::size_t >( BOOST_FIBERS_SPIN_BEFORE_YIELD), 2 * prev_count + 10);
            ...
            if ( spinlock_status::locked == hle_exchange( state_, spinlock_status::locked) ) {
                ...
                count_.store( prev_count + (count - prev_count) / 8, std::memory_order_relaxed);
...
#endif // BOOST_FIBERS_SPINLOCK_TTAS_ADAPTIVE_HLE_H
